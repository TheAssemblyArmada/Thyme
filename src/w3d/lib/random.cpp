...
unsigned int Random3Class::m_mix1[20] = { 0x0baa96887,
    0x01e17d32c,
    0x003bcdc3c,
    0x00f33d1b2,
    0x076a6491d,
...
unsigned int Random3Class::m_mix2[20] = { 0x04b0f3b58,
    0x0e874f0c3,
    0x06955c5a6,
    0x055a7ca46,
    0x04d9a9d86,
...
int Random3Class::operator()()
{
    ...
    for (int i = 0; i < 4; i++) {
        int hihold = hiword;
        int temp = hihold ^ m_mix1[i];
        int itmpl = temp & 0xffff;
        int itmph = temp >> 16;
        temp = itmpl * itmpl + ~(itmph * itmph);
        temp = (temp >> 16) | (temp << 16);
        hiword = loword ^ ((temp ^ m_mix2[i]) + itmpl * itmph);
        loword = hihold;
    }
