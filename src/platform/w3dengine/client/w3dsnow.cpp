...
void W3DSnowManager::Render(RenderInfoClass &rinfo)
{
    ...
    if (g_theWeatherSetting->m_snowEnabled && m_isVisible) {
        ...
        AABoxClass aabox;
        g_theTerrainRenderObject->Get_Maximum_Visible_Box(rinfo.m_camera.Get_Frustum(), &aabox);
...
// refactor of the code below this
...
void W3DSnowManager::Render_Sub_Box(RenderInfoClass &rinfo, int x1, int y1, int x2, int y2)
{
    ...
    if (x_delta > m_unk5) {
        if (y_delta > m_unk5) {
            ...
            min_z = m_unk6 - m_boxDimensions;
            min_y = (float)(y_offset + y1) * m_boxDensity - m_unk9;
            min_x = (float)x1 * m_boxDensity - m_unk9;
            minmax_aabox.m_minCorner.Set(min_x, min_y, min_z);

            max_y = (float)y2 * m_boxDensity + m_unk9;
            max_x = (float)(x_offset + x1) * m_boxDensity + m_unk9;
            minmax_aabox.m_maxCorner.Set(max_x, max_y, m_unk6);

            if (CollisionMath::Overlap_Test(camera.Get_Frustum(), AABoxClass(minmax_aabox)) != CollisionMath::OUTSIDE) {
                Render_Sub_Box(rinfo, x1, y_offset + y1, x_offset + x1, y2);
            }

            min_z = m_unk6 - m_boxDimensions;
            min_y = (float)(y_offset + y1) * m_boxDensity - m_unk9;
            min_x = (float)(x_offset + x1) * m_boxDensity - m_unk9;
            minmax_aabox.m_minCorner.Set(min_x, min_y, min_z);

            max_y = (float)y2 * m_boxDensity + m_unk9;
            max_x = (float)x2 * m_boxDensity + m_unk9;
            minmax_aabox.m_maxCorner.Set(max_x, max_y, m_unk6);

            if (CollisionMath::Overlap_Test(camera.Get_Frustum(), AABoxClass(minmax_aabox)) != CollisionMath::OUTSIDE) {
                Render_Sub_Box(rinfo, x_offset + x1, y_offset + y1, x2, y2);
            }

            min_z = m_unk6 - m_boxDimensions;
            min_y = (float)y1 * m_boxDensity - m_unk9;
            min_x = (float)x1 * m_boxDensity - m_unk9;
            minmax_aabox.m_minCorner.Set(min_x, min_y, min_z);

            max_y = (float)(y_offset + y1) * m_boxDensity + m_unk9;
            max_x = (float)(x_offset + x1) * m_boxDensity + m_unk9;
            minmax_aabox.m_maxCorner.Set(max_x, max_y, m_unk6);

            if (CollisionMath::Overlap_Test(camera.Get_Frustum(), AABoxClass(minmax_aabox)) != CollisionMath::OUTSIDE) {
                Render_Sub_Box(rinfo, x1, y1, x_offset + x1, y_offset + y1);
            }
            ...
            if (CollisionMath::Overlap_Test(camera.Get_Frustum(), AABoxClass(minmax_aabox)) != CollisionMath::OUTSIDE) {
                Render_Sub_Box(rinfo, x_offset + x1, y1, x2, y_offset + y1);
            }
            ...
            if (CollisionMath::Overlap_Test(camera.Get_Frustum(), AABoxClass(minmax_aabox)) != CollisionMath::OUTSIDE) {
                Render_Sub_Box(rinfo, x1, y1, x_offset + x1, y2);
            }
            ...
            if (CollisionMath::Overlap_Test(camera.Get_Frustum(), AABoxClass(minmax_aabox)) != CollisionMath::OUTSIDE) {
                Render_Sub_Box(rinfo, x_offset + x1, y1, x2, y2);
            }
        }
    } else if (y_delta > m_unk5) {
        ...
        if (CollisionMath::Overlap_Test(camera.Get_Frustum(), AABoxClass(minmax_aabox)) != CollisionMath::OUTSIDE) {
            Render_Sub_Box(rinfo, x1, y_offset + y1, x2, y2);
        }
        ...
        if (CollisionMath::Overlap_Test(camera.Get_Frustum(), AABoxClass(minmax_aabox)) != CollisionMath::OUTSIDE) {
            Render_Sub_Box(rinfo, x1, y1, x2, y_offset + y1);
        }
...
{
#if 0
    // for quick testing render as quads
    // expected behavior is that snowflakes can be massive,
    // rain seems remains same size
    *(bool *)&g_theWeatherSetting->m_snowPointSprites = false;
#endif
