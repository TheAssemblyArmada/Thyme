...
bool MotionChannelClass::Load_W3D(ChunkLoadClass &cload)
{
    ...
    uint32_t chunk_size = cload.Cur_Chunk_Length() - sizeof(chan);
    ...
    uint32_t bytes = 4 * m_vectorLen * (m_lastFrame - m_firstFrame + 1);
    uint32_t bytesleft = bytes - 4;
...
bool BitChannelClass::Load_W3D(ChunkLoadClass &cload)
{
    ...
    uint32_t chunk_size = cload.Cur_Chunk_Length();
    if (cload.Read(&chan, sizeof(chan)) != sizeof(chan)) {
        return false;
    }
    ...
    uint32_t bytes = (m_lastFrame - m_firstFrame + 8) >> 3;
    uint32_t bytesleft = bytes - 1;
...
bool TimeCodedMotionChannelClass::Load_W3D(ChunkLoadClass &cload)
{
    ...
    m_pivotIdx = chan.Pivot;
    m_cachedIdx = 0;
    m_data = new uint32_t[((chunk_size / sizeof(uint32_t)) + 1)];
    m_data[0] = chan.Data[0];
...
void TimeCodedMotionChannelClass::Get_Vector(float frame, float *setvec)
{
    ...
    if (index == m_packetSize * (m_numTimeCodes - 1)) {
        ...
        int32_t index2 = m_packetSize + index;
        uint32_t val = m_data[index2];
...
Quaternion TimeCodedMotionChannelClass::Get_Quat_Vector(float frame_idx)
{
    ...
    Quaternion q1(true);
    int32_t index = Get_Index(frame_idx);
...
uint32_t TimeCodedMotionChannelClass::Get_Index(uint32_t timecode)
{
    ...
    if (timecode < Get_Frame_From_Data(m_data[m_cachedIdx])) {
        uint32_t result = TimeCodedMotionChannelClass::Binary_Search_Index(timecode);
        m_cachedIdx = result;
        return result;
    }
    ...
    uint32_t result = m_packetSize + m_cachedIdx;
    ...
    if (result != m_lastTimeCodeIdx) {
        int32_t index = result + m_packetSize;
...
uint32_t TimeCodedMotionChannelClass::Binary_Search_Index(uint32_t timecode) const
{
    int32_t count2;
    int32_t count = 0;
    int32_t rightIdx = m_numTimeCodes - 2;
    uint32_t result = m_lastTimeCodeIdx;
...
bool TimeCodedBitChannelClass::Load_W3D(ChunkLoadClass &cload)
{
    ...
    Free();
    const uint32_t chunk_size = cload.Cur_Chunk_Length();
    ...
    m_defaultVal = chan.DefaultVal;
    m_cachedIdx = 0;
    const int32_t bytesleft = 4 * m_numTimeCodes - 4;

    captainslog_assert((sizeof(chan) + bytesleft) == chunk_size);

    m_bits = new uint32_t[m_numTimeCodes];
...
int32_t TimeCodedBitChannelClass::Get_Bit(int32_t frame)
{
    ...
    if (frame >= static_cast<int32_t>(Get_Frame_From_Data(m_bits[m_cachedIdx]))) {
        count = m_cachedIdx + 1;
    }

    while (count < m_numTimeCodes && frame >= static_cast<int32_t>(Get_Frame_From_Data(m_bits[count]))) {
        ++count;
    }
...
void AdaptiveDeltaMotionChannelClass::Get_Vector(float frame, float *setvec)
{
    ...
    *setvec = GameMath::Lerp(value1, value2, frame - (uint32_t)frame);
}
...
float AdaptiveDeltaMotionChannelClass::Get_Frame(uint32_t frame_idx, uint32_t vector_idx)
{
    float dst[4];
