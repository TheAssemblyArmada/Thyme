...
StreakLineClass::StreakLineClass() : m_maxSubdivisionLevels(0), m_normalizedScreenArea(0), m_unk(nullptr) {}
...
{
    captainslog_assert(count >= 2 && locs != nullptr);
    m_pointLocations.Delete_All();
    Invalidate_Cached_Bounding_Volumes();
    if (count != 0 && locs != nullptr) {
...
{
    captainslog_assert(count >= 2 && widths != nullptr);
...
{
    captainslog_assert(count >= 2 && colors != nullptr);
    m_pointColors.Delete_All();
    Invalidate_Cached_Bounding_Volumes();
    if (count != 0 && colors != nullptr) {
        for (unsigned int i = 0; i < count; ++i) {
...
// needs StreakRendererClass Render functions
#if 0
