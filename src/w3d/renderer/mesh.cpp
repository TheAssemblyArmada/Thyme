...
void MeshClass::Render_Material_Pass(MaterialPassClass *pass, IndexBufferClass *ib)
{
    ...
    if (m_model->Get_Flag(MeshModelClass::SKIN)) {
        ...
        if (m_opacityOverride == 1.0f || pass->Peek_Material() == nullptr) {
            opacity = -1.0f;
        } else {
            opacity = pass->Peek_Material()->Get_Opacity();
            pass->Peek_Material()->Set_Opacity(m_opacityOverride);
        }
...
bool MeshClass::Cast_Ray(RayCollisionTestClass &raytest)
{
    ...
    if (raytest.m_ignoreTranslucentMeshes && Is_Alpha() != 0) {
        return false;
    }

    if (raytest.m_ignoreHiddenMeshes && Is_Hidden() != 0) {
        return false;
    }
