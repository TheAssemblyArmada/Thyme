...
//
//   Description:: 4x3 Matrix class with an implicit 4th row.
//
...
int Matrix3D::Is_Orthogonal() const
{
    ...
    if ( Vector3::Dot_Product(x, y) > GAMEMATH_EPSILON ) return 0;
    if ( Vector3::Dot_Product(y, z) > GAMEMATH_EPSILON ) return 0;
    if ( Vector3::Dot_Product(z, x) > GAMEMATH_EPSILON ) return 0;
    if ( GameMath::Fabs(x.Length2() - 1.0f) > GAMEMATH_EPSILON ) return 0;
    if ( GameMath::Fabs(y.Length2() - 1.0f) > GAMEMATH_EPSILON ) return 0;
    if ( GameMath::Fabs(z.Length2() - 1.0f) > GAMEMATH_EPSILON ) return 0;
...
{
    return GameMath::Atan2(Row[2][1], Row[1][1]);
}

float Matrix3D::Get_Y_Rotation() const
{
    return GameMath::Atan2(Row[0][2], Row[2][2]);
}

float Matrix3D::Get_Z_Rotation() const
{
    return GameMath::Atan2(Row[1][0], Row[0][0]);
}
...
void Matrix3D::Look_At(const Vector3 &p, const Vector3 &t, float roll)
{
    ...
    float len = (float)GameMath::Sqrt(rad2);
    ...
    if ( rad2 != 0.0f ) {
        float inv_len2 = (float)GameMath::Inv_Sqrt(rad2);
        sinp = dz * inv_len2;
        cosp = len * inv_len2;
    } else {
        sinp = 0.0f;
...
void Matrix3D::Transform_Center_Extent_AABox(const Vector3 &center, const Vector3 &extent, Vector3 *set_center, Vector3 *set_extent) const
{
    for ( int i = 0; i < 3; i++ ) {
        ...
        for ( int j = 0; j < 3; j++ ) {
            ...
            (*set_extent)[i] += GameMath::Fabs(Row[i][j] * extent[j]);
        }
    }
}
